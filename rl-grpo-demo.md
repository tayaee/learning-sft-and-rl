...
> merge된 base+LoRA 모델이 있어야 `query_rl_grpo.py`로 추론할 수 있습니다.
...
# uv run query_rl_grpo.py --mode full "$Q"
```
...
이 4 모델 비교가 끝났다면 → Stage 9 (`./rl-grpo-09-lm-eval-rl-grpo.sh full`) 로 정량 평가.
