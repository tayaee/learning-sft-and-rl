...
> DPO 데이터(`data/<mode>/train_rl-dpo.jsonl`)가 이미 있으면 데이터 재생성 없이 바로 시작 가능.
...
| 모드 | 데이터 | steps | 소요 | 목적 |
|------|--------|-------|------|------|
| **smoke** | 20 pairs | max_steps 3 | ~5–10분 | 파이프라인 점검 (데이터/학습/merge/추론 전 경로) |
...
# DPO 데이터가 없다면: ./rl-dpo-01-make-rl-dpo-data.sh smoke
...
```bash
./rl-orpo-02-sanity-check-orpo-data.sh smoke
```

**기대**: chosen 은 `` 블록 + `###` 구조 + 결론 단정, rejected 는 평평하거나 짧은 응답.
포맷은 passthrough 로 전달되는 메시지 리스트(TRL conversational) 포맷
(`chosen`/`rejected` 가 `[user, assistant]` 배열). smoke 파일은 20줄.

### A-3. smoke 학습 (3 step)

```bash
./rl-orpo-04-run-axolotl.sh smoke        # configs/qwen2.5-1.5b-rl-orpo-smoke.yaml
```
...
- 완료 후 `outputs/qwen2.5-1.5b-rl-orpo-smoke/` 에 어댑터 저장 확인

### A-4. smoke merge + 추론

```bash
./rl-orpo-05-merge.sh smoke              # outputs/qwen2.5-1.5b-rl-orpo-smoke-merge/merged/
./rl-orpo-06-test-model.sh smoke         # 간단한 추론 한 번
```
...
```bash
./rl-orpo-01-make-orpo-data.sh full      # data/full/train_rl-orpo.jsonl = 1000 lines
wc -l data/full/train_rl-orpo.jsonl      # 1000 확인
```

> 직접 rejection sampling 으로 새로 만들고 싶으면:
> `uv run python make_rl_orpo_data.py --generate --num-prompts 1000 \
>   --out data/full/train_rl-orpo.jsonl`
...
**핵심 파라미터** (`configs/qwen2.5-1.5b-rl-orpo.yaml`):
...
(smoke 로도 `--mode smoke --num-prompts 10` 실행 가능하지만 3-step 학습이라 개선 폭은 미미하다.)
...
결과는 `outputs/lm_eval_results/rl-orpo-full/` (smoke 는 `rl-orpo-smoke/`). sft-demo 의 base/SFT 점수와 나란히 비교.
...
| `configs/qwen2.5-1.5b-rl-orpo.yaml` | full run config |
| `configs/qwen2.5-1.5b-rl-orpo-smoke.yaml` | smoke test config |
| `orpo_passthrough_plugin.py` | axolotl 0.18 에 빠진 orpo/passthrough 전략 등록 플러그인 |
| `rl-orpo-01..09-*.sh` | 단계별 드라이버 — 전부 `$1=smoke\|full` 필수. 데이터는 모드별 디렉터리(`data/smoke/`, `data/full/`)로 분리. 각 스크립트마다 `<script>-smoke.sh`/`<script>-full.sh` 래퍼 있음 |
| `query_rl_orpo.py` | ORPO merge 모델 추론 (--mode smoke/full) |
...
```bash
# 점검 (10분)
./rl-orpo-01-make-orpo-data.sh smoke && ./rl-orpo-04-run-axolotl.sh smoke && \
...
## E. axolotl 0.18 ORPO 관련 검증 노트 (smoke test 로 확인)
...
| OOM (DGX Spark 통합메모리) | smoke 처럼 `micro_batch_size: 1` 로, `sequence_len: 768` 로 축소 |
...
| merge 후 추론에서 모델 없음 에러 | `--mode` 플래그가 smoke/merge 모드와 일치하는지 확인 |
