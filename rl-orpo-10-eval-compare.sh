#!/bin/bash
# rl-orpo-10: 이전 모델(SFT merge) vs 새 모델(ORPO merge) 비교 평가 — 2개 평가 + 비교표
#   $1 = smoke | full (반드시 지정)
...
if [ "$MODE" = "smoke" ]; then
  ...
  NEW_MODEL=./outputs/qwen2.5-1.5b-rl-orpo-smoke-merge/merged
  PREV_LABEL="SFT(smoke)"
  NEW_LABEL="ORPO(smoke)"
else
  PREV_MODEL=./outputs/qwen2.5-1.5b-sft-merge/merged         # 이전 모델 = SFT(full)
  NEW_MODEL=./outputs/qwen2.5-1.5b-rl-orpo-merge/merged
  PREV_LABEL="SFT(full)"
  NEW_LABEL="ORPO(full)"
fi
...
[ -d "$NEW_MODEL" ] || { echo "ERROR: $NEW_MODEL 없음 — 먼저 rl-orpo-04 + rl-orpo-05 을 --mode $MODE 로 실행하세요." >&2; exit 1; }
